...
const ChatWindown = () => {
  ...
  const [inputMessage, setInputMessage] = useState({
    messageType: 'text', // 0: text, 1: image, 2: file, 4: video, 5: audio
    content: '',
  });
  ...
  const getLastLoginMessage = (lastLogout) => {
    ...
    if (diffInSec < 60) {
      return `Truy cập ${diffInSec} giây trước`;
    } else if (diffInMin < 60) {
      return `Truy cập ${diffInMin} phút trước`;
    } else if (diffInHours < 24) {
      return `Truy cập ${diffInHours} giờ trước`;
    } else {
      return `Truy cập ${logoutTime.toLocaleDateString('vi-VN')}`; // Format as dd/MM/yyyy
    }
  };
            ...
            <div className="bg-white border-b border-gray-200 p-4 flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {isFriendOnline(conversation.memberUserIds.find(v => v !== userLogin.id)) ? 'Đang hoạt động' :
          ...
          <div className="text-center text-gray-500">
            {
              ...
              (
                messages.map((message, index) => {
                  return (
                    <div key={index} className={`flex ${message.senderId === userLogin.id ? 'justify-end' : 'justify-start'} items-end mt-4`}>
          ...
          <form className="flex items-center space-x-2">
            ...
            />
            {
              isSending ? <div className="w-6 h-6 border-t-2 border-b-2 border-blue-500 rounded-full animate-spin"></div> : <button onClick={handlerSendMessage}
                ...
                type="button" className="p-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition">
