...
export const InAppNavigation = () => {
    ...
    if (isPhoneNumber(search)) {
      try {
        ...
        if (result && result.data) {
          const friendId = [result.data.friendId, result.data.userId].filter(
            (id) => id !== userLogin.id
          )[0];
          const friendData = {
            userId: userLogin.id,
            ...
            friendInfo: {
              ...result.data,
              accountId:
                result.data.friendId || result.data.id || result.data.accountId,
  ...
  useEffect(() => {
    ...
    socket.on(
      "receive_update_profile_group",
      handlerReceiveUpdatedConversation
    );
  ...
  useEffect(() => {
    const handleUpdateMessage = async (data) => {
      ...
      await dispatch(
        updateLastMessage({ conversationId: cvt.id, message: ms })
      );
    };
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off(
        "receive-reject-friend-for-me",
        handleRejectFriendRequestForMe
      );
    };
  }, []);
  ...
  useEffect(() => {
    const handleReceiveFriendRequest = async (data) => {
      ...
      showToast(
        ...
        "top",
        "Thông báo",
        "Bạn có lời mời kết bạn mới từ " + data.fullName
      );
  ...
  useEffect(() => {
    ...
    socket.on(
      "receive-accept-friend-for-me",
      handleReceiveAcceptFriendRequestForMe
    );
    return () => {
      socket.off(
        "receive-accept-friend-for-me",
  ...
  const handleUnfriend = async (item) => {
    try {
      ...
      const friendId = [item.friendId, item.userId].filter(
        (id) => id !== userLogin.id
      )[0];
      ...
      showToast(
        ...
        "top",
        "Lỗi",
        "Đã xảy ra lỗi khi xóa kết bạn. Vui lòng thử lại."
      );
    }
  };
  ...
  const handlerSendFriend = async (item) => {
    const request = {
      ...
      contentRequest:
        "Mình tìm kiếm bạn qua số điện thoại. Kết bạn với mình nhé!",
    };
    ...
    <SafeAreaView style={{ flex: 1, flexDirection: "column" }}>
            ...
            >
              ...
              >
                <IconIO
                  ...
                  size={18}
                  color="blue"
                  style={{ marginRight: 10, width: 24, textAlign: "center" }}
                />
      ...
      {search.length > 0 && (
        <View>
          ...
          {isFinding && <ActivityIndicator size="small" color="#000" />}
          {searchResult && (
            <>
              <FlatList
                style={{ height: "100%" }}
